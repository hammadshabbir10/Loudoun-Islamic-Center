...
Also set a unique `name` and R2 `bucket_name` in `wrangler.jsonc` per site (see step 3).
...
Each command prompts for the value. Local dev uses `.dev.vars` (copy `.dev.vars.example`), which is gitignored. The default test Turnstile keys pass locally but **must** be replaced with production keys here.
