...
3. **Action** — `src/actions/index.ts`, the Astro Action that runs the pipeline server-side on the Worker.

## 1. Reuse / adapt the island

Start from `src/components/ContactForm.tsx`. It already renders the shadcn form fields, mounts the Turnstile widget (site key from `TURNSTILE_SITE_KEY` in `@/config`), calls the action via the `astro:actions` client, and shows success/error toasts with `sonner` (the `<Toaster />` is mounted once in the layout).
...
3. The action receives the new field automatically once it's in the schema.
...
The action lives in `src/actions/index.ts` and validates its `input` with `leadSchema`. It reads the Worker env from **`context.locals.runtime.env`** (typed via the global `Env` interface in `src/env.d.ts`).
