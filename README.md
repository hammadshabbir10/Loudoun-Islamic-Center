...
> **Secrets rule:** `TURNSTILE_SECRET`, `PLUNK_API_KEY`, and `Z360_TOKEN` live only in Worker
> Secrets (prod) / the gitignored `.dev.vars` (local), and are read at runtime from
> `context.locals.runtime.env`. Never `import.meta.env`, never `process.env`, never hardcoded.
...
| `pnpm cf-typegen`   | Regenerate Worker binding types (`wrangler types`).      |

Before a PR: `pnpm check && pnpm typecheck && pnpm lint && pnpm test && pnpm build`. CI must be
green to merge.
